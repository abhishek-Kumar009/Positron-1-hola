...
router.get('/add-unfollowing/:question_id',required,(req,res)=>{
    Question.findById(req.params.question_id,(error,question)=>{
        ...
        User.findOneAndUpdate({_id:req.user._id},{$pull: {following:question._id }},(error,user)=>{
            if(error)
                return ErrorHandler(error,req,res);
            else    
                console.log(user);
                return res.status(200).end("Question Unfollowed");
        })  
    })
})
